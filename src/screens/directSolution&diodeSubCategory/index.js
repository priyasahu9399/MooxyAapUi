...
const DirectSolutionDiodeSubCategory = ({navigation  , route}) => {
  ...
  navigation.setOptions({
    headerTitle: () => (
      <View style={styles.headerrow}>
        ...
        />
        <Text style={styles.headerheading}>{pretitle?.pretitle}</Text>
        <Image
          source={icons.rarrow}
          resizeMode="contain"
          style={styles.threeicon}
              ...
              <FlatList
                ...
                renderItem={({item, index}) => (
                  <TouchableOpacity
                    onPress={() => {
                      setActive(index),
                        navigation.navigate('DirectSolutionDiodeSubCategory2', {
                          data: {
                            pretitle: pretitle,
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(DirectSolutionDiodeSubCategory);
